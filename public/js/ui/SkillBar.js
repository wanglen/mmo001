...
import { CONSUMABLE_KIND, countPotionsByKind, findFirstPotionIndex, canQuickUsePotion } from '/shared/consumables.js';
...
export class SkillBar {
  ...
  updateSkillSlots(player) {
    for (let i = 0; i < SKILL_HOTKEY_COUNT; i++) {
      ...
      mp.textContent = `${skill.mpCost}`;
      slot.title = `${skill.name} — ${skill.mpCost} MP`;
      slot.setAttribute('aria-label', `${skill.name}, ${skill.mpCost} MP, key ${i + 1}`);
