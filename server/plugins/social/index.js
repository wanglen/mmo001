...
export function onSocialPlayerJoined(playerId, ctx) {
  broadcastOnlinePlayers(ctx.io, ctx.playerManager);
  ctx.io
    .to(playerId)
    .emit(EVENTS.PARTY_STATE, ctx.partyManager.getPartyStateForPlayer(playerId, ctx.playerManager));
}

/** @param {string} playerId @param {import('../../../shared/plugins/types.js').ServerContext} ctx */
export function onSocialDisconnect(playerId, ctx) {
  const { io, playerManager, partyManager } = ctx;
